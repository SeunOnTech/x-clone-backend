...
/* -------------------------------------------------------------
   🆕 NEW CRISIS GUIDELINE PROMPT
------------------------------------------------------------- */
const CRISIS_GUIDELINE = `
A false claim is circulating suggesting that Zenith Bank's strong financial results are linked to improper or unethical lending practices. This kind of misinformation creates doubt and can make the public suspect excessive risk-taking or hidden instability.

A crisis-style misinformation post should capture the emotional tension of the moment — uncertainty, suspicion, fear, or worry. It should sound like someone reacting to the rumor, amplifying it, or expressing public anxiety about Zenith Bank’s financial behavior.

The goal is NOT to output this paragraph. The goal is to use it as internal context to shape a short misinformation-style social media post that:
- mentions “Zenith Bank”
- reflects public fear, doubt, or suspicion
- sounds like a real Nigerian tweet reacting to alarming news
- stays within 30–50 words
- uses the selected language style (English or Pidgin)
`;
...
/* -------------------------------------------------------------
   🆕 UPDATED CRISIS CONTENT GENERATOR USING GUIDELINE
------------------------------------------------------------- */
async function generateCrisisMainPostContent(
  ...
  const langStyle =
    language === "PIDGIN"
      ? 'Write fully in Nigerian Pidgin, with natural Nigerian slang.'
      : "Write ONLY in clean English.";

  const prompt = `
Use the following scenario as INTERNAL CONTEXT ONLY. 
Do NOT output it directly:

"${CRISIS_GUIDELINE}"

Now create a misinformation-style Nigerian tweet reacting emotionally to the rumor.
...
- ${langStyle}
- Length: 30–50 words.
- Must sound like a real person's reaction to the rumor.
